...
uses
  ...
  JvExMask, JvSpin, ActnList, XPStyleActnCtrls, ActnMan;
...
for i:=0 to 6 do self.BlankBox.Items.Add(blanknames[i]);
...
self.Caption:='Параметры - '+self.BlankBox.Text;
self.ComboBox1.Items.Assign(screen.Fonts);
self.GroupBox3.Visible:=self.BlankBox.ItemIndex=2;
...
for i:=1 to self.ComponentsGrid.ColCount do self.ComponentsGrid.Cells[i,1]:='';
...
self.ComponentsGrid.Cells[1,0]:='Надпись';
...
self.Button3.Click;
self.CheckBox13.Checked:=false;
self.SpinEdit1.value:=100;
self.SpinEdit2.value:=100;
self.SpinEdit3.value:=0;
self.SpinEdit4.value:=0;
self.SpinEdit5.value:=130;
self.SpinEdit6.value:=100;
self.SpinEdit7.value:=0;
self.SpinEdit8.value:=0;
self.SpinEdit9.value:=100;
self.SpinEdit10.value:=100;
self.SpinEdit11.value:=0;
self.SpinEdit12.value:=0;
...
procedure TSettingsForm.TabSheet1Show(Sender: TObject);
begin
self.Caption:='Параметры';
...
procedure TSettingsForm.Button10Click(Sender: TObject);
var i:integer;
mi,x,xmin1,xmax1,xmin2,xmax2,ymax,ymin:extended;
xso,hs,hso,xs,xsnew,ys,yso,ysnew,ysonew:array[1..100] of extended;

s:string;
begin

ymax:=(((Tfrxreportpage(mainform.frxReport1.Pages[1]).paperheight-(Tfrxreportpage(mainform.frxReport1.Pages[1]).topMargin+Tfrxreportpage(mainform.frxReport1.Pages[1]).bottomMargin))));
xmin1:=100000;
xmax1:=0;
for i:=1 to self.ComponentsGrid.RowCount do
begin
s:=removespec(self.ComponentsGrid.Cells[1,i]);
if copy(s,1,length('[Дисциплина_назв_'))='[Дисциплина_назв_' then
begin
ys[strtoint(copy(s,length('[Дисциплина_назв_')+1,length(s)-length('[Дисциплина_назв_')-1))]:=strtofloat(self.ComponentsGrid.Cells[3,i]);
hs[strtoint(copy(s,length('[Дисциплина_назв_')+1,length(s)-length('[Дисциплина_назв_')-1))]:=strtofloat(self.ComponentsGrid.Cells[5,i]);
x:=strtofloat(self.ComponentsGrid.Cells[2,i]);
xs[strtoint(copy(s,length('[Дисциплина_назв_')+1,length(s)-length('[Дисциплина_назв_')-1))]:=x;
if xmax1<x then xmax1:=x;
if xmin1>x then xmin1:=x;

end;

if copy(s,1,length('[Дисциплина_оценка_'))='[Дисциплина_оценка_' then
begin
yso[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]:=strtofloat(self.ComponentsGrid.Cells[3,i]);
hso[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]:=strtofloat(self.ComponentsGrid.Cells[5,i]);
x:=strtofloat(self.ComponentsGrid.Cells[2,i]);
xso[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]:=x;
if xmax2<x then xmax2:=x;
if xmin2>x then xmin2:=x;
...
begin
 ysnew[i]:=mi*(i-1);
if (ysnew[i]+hs[i]>ymax) then
begin
ysnew[i]:=ysnew[i]-ymax;
//xs[i]:=xmax1;
end;
if (ysnew[i]<ymin) and (xs[i]>=xmax1) then ymin:=ysnew[i];

 ysonew[i]:=mi*(i-1);
if (ysonew[i]+hso[i]>ymax) then
begin
ysonew[i]:=ysonew[i]-ymax;
//xso[i]:=xmax2;
end;
...
if (xs[i]>=xmax1) then
begin
ysnew[i]:=ysnew[i]-ymin+ysnew[1];
end;
if (xso[i]>=xmax2) then
begin
ysonew[i]:=ysonew[i]-ymin+ysonew[1];
end;
...
if copy(s,1,length('[Дисциплина_назв_'))='[Дисциплина_назв_' then
begin
self.ComponentsGrid.Cells[3,i]:=format('%.1f',[ysnew[strtoint(copy(s,length('[Дисциплина_назв_')+1,length(s)-length('[Дисциплина_назв_')-1))]]);
self.ComponentsGrid.Cells[2,i]:=format('%.1f',[xs[strtoint(copy(s,length('[Дисциплина_назв_')+1,length(s)-length('[Дисциплина_назв_')-1))]]);

end;

if copy(s,1,length('[Дисциплина_оценка_'))='[Дисциплина_оценка_' then
begin
self.ComponentsGrid.Cells[3,i]:=format('%.1f',[ysnew[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]]);
self.ComponentsGrid.Cells[2,i]:=format('%.1f',[xso[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]]);

end;

{if copy(s,1,length('[Дисциплина_оценка_'))='[Дисциплина_оценка_' then
begin
yso[strtoint(copy(s,length('[Дисциплина_оценка_')+1,length(s)-length('[Дисциплина_оценка_')-1))]:=strtofloat(self.ComponentsGrid.Cells[2,i]);
end;}

end;
// self.Button3.Click;
//showmessage(floattostr(y2-y1));
end;

procedure TSettingsForm.Button11Click(Sender: TObject);
var i:integer;
mi,x,xmin1,xmax1,xmin2,xmax2,ymax,ymin:extended;
xso,hs,hso,xs,xsnew,ys,yso,ysnew,ysonew:array[1..100] of extended;
